...
const CertificationCard: React.FC<{ cert: Certification; index: number }> = ({ cert, index }) => {
  return (
    <motion.a
      ...
      className="glass group p-8 rounded-[2.5rem] flex flex-col h-full hover:bg-white/[0.04] hover:border-white/20 transition-all duration-500 relative overflow-hidden"
    >
      ...
      <div className="flex items-start gap-6 mb-8">
        ...
        <div className="flex-1">
          <span className="text-[9px] font-bold text-white/30 uppercase tracking-[0.3em] mb-2 block">{cert.issuer}</span>
          <h3 className="text-xl font-bold tracking-tight text-white leading-tight group-hover:text-blue-400 transition-colors duration-500">
            {cert.title}
          </h3>
        </div>
      </div>
      ...
      <div className="mt-auto pt-6 border-t border-white/5 space-y-2">
        <div className="flex justify-between items-center text-[10px] font-medium tracking-wider">
          <span className="text-white/40 uppercase">Issued</span>
          <span className="text-white/60">{cert.date}</span>
        </div>
        {cert.credentialId && (
          <div className="flex justify-between items-center text-[10px] font-medium tracking-wider">
            <span className="text-white/40 uppercase">ID</span>
            <span className="text-white/60 truncate ml-4 max-w-[150px]">{cert.credentialId}</span>
          </div>
        )}
      </div>
    ...
    <section className="py-48 px-6 bg-black">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-32 space-y-6">
          <span className="text-white/40 font-bold uppercase tracking-[0.6em] text-[10px]">Accreditations</span>
          <h2 className="text-6xl md:text-8xl font-bold tracking-tighter leading-none">
            QUALIFIED <br /> <span className="text-white/30 italic">FOUNDATIONS</span>
          </h2>
        </div>
